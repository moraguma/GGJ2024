...
# --------------------------------------------------------------------------------------------------
# Physics
const CHECKING_DISTANCE = 32
const SPAWNING_DISTANCE = 32
...
func _get_gravity_velocity():
	return Vector2(0, -1) * JUMP_SPEED if Input.is_action_just_pressed("jump") and jump_checker.is_colliding() else super()
